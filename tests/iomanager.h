...
public:
     ...
     * 是为用户提供的接口，如在服务端的 accept 中，要接听fd,待返回 newfd后，就说明
     * 建立成功了，accept这里就要监听，用epoll_ctlzhi添加监听，并设置回调函数为newfd的读事件，
     * 回调函数就是用户传过来的cb。
     */
...
protected:
    /**
     * @brief 通知调度器有任务要调度
     * @details 写pipe让idle协程从epoll_wait退出，待idle协程yield
     * 之后Scheduler::run就可以调度其他任务
     */
    void tickle() override;

    /**
     * @brief 判断是否可以停止
     * @details 判断条件是Scheduler::stopping()外加IOManager的
     * m_pendingEventCount为0，表示没有IO事件可调度了
     */
    bool stopping() override;

    /**
     * @brief idle协程
     * @details 对于IO协程调度来说，应阻塞在等待IO事件上，idle退出的时机
     * 是epoll_wait返回，对应的操作是tickle或注册的IO事件发生
     */
    void idle() override;
